...
export default function CheckoutModal({ onClose, onSuccess }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      {step !== 2 && (
        <div
          className="relative w-full max-w-lg rounded-2xl shadow-2xl border border-white/20 dark:border-white/10 overflow-hidden animate-modal-in"
        ...
        >
          ...
          {step === 1 && (
            <>
              <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200/70">
                <h3 className="text-base font-bold text-gray-900">Review & Checkout</h3>
              ...
              <div className="p-5 space-y-4 max-h-[60vh] overflow-y-auto">
                ...
                <div className="flex items-center gap-3 bg-gray-50/80 rounded-xl p-3">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-gray-900">{customer?.name || 'Walk-in Customer'}</p>
                ...
                <div className="space-y-1">
                  ...
                  {items.length > 5 && (
                    <p className="text-xs text-gray-400 py-1">+{items.length - 5} more items…</p>
                  )}
                </div>
                ...
                <div className="bg-indigo-50/60 dark:bg-indigo-900/20 rounded-xl p-4 space-y-2">
                  ...
                  {isIgst
                    ? <div className="flex justify-between text-sm text-gray-500"><span>IGST</span><span>₹{totalIgst.toFixed(2)}</span></div>
                    : <>
                        <div className="flex justify-between text-sm text-gray-500"><span>CGST</span><span>₹{totalCgst.toFixed(2)}</span></div>
                        <div className="flex justify-between text-sm text-gray-500"><span>SGST</span><span>₹{totalSgst.toFixed(2)}</span></div>
                      </>
                  }
                  {invDisc > 0 && (
                  ...
                  <div className="border-t border-indigo-200/60 pt-2 flex justify-between font-bold text-base">
              ...
              <div className="px-5 pb-5 flex gap-3">
                <button onClick={onClose} className="btn-secondary text-sm">Back</button>
                <button onClick={() => setStep(2)} className="btn-primary text-sm flex-1 gap-2">
          ...
          {step === 3 && (
            <>
              <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200/70">
                <h3 className="text-base font-bold text-gray-900">Confirm Invoice</h3>
                <button onClick={() => setStep(1)} className="btn-icon w-7 h-7 p-1 text-gray-400"><X size={16} /></button>
              </div>
              <div className="p-5 space-y-4">
                ...
                <div className="space-y-2">
                  {payments.map((p, i) => (
                    <div key={i} className="flex justify-between text-sm">
                      <span className="text-gray-600 capitalize">{p.mode}</span>
                  ...
                  {Math.abs(balance) > 0.01 && (
                    <div className="flex justify-between text-sm">
                      <span className={balance > 0 ? 'text-orange-600' : 'text-blue-600'}>
                        {balance > 0 ? 'Balance Due' : 'Change'}
                      </span>
                      <span className={`font-semibold ${balance > 0 ? 'text-orange-600' : 'text-blue-600'}`}>
                ...
                <div className="flex gap-3">
                  <button onClick={() => setStep(2)} className="btn-secondary text-sm">Change Payment</button>
                  <button
                    onClick={handleSubmit}
                    disabled={saving}
                    className="btn-success text-sm flex-1 gap-2 font-bold"
                  >
                    {saving ? (
          ...
          {step === 4 && (
            <div className="p-8 text-center">
              ...
              <h3 className="text-xl font-bold text-gray-900 mb-1">Invoice Created!</h3>
              {savedSale?.invoice_number && (
                <p className="text-sm text-indigo-600 font-semibold mb-1">{savedSale.invoice_number}</p>
              )}
              <p className="text-sm text-gray-500 mb-6">PDF opened in a new tab</p>
